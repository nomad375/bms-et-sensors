...
def draw_led_patterns(draw, x, y, width, patterns):
    draw_text(draw, (x, y), "BLUE LED STATUS", "#637083", 28, True)
    entries = []
    for item in patterns:
        if ":" in item:
            pattern, meaning = item.split(":", 1)
            entries.append((pattern.strip(), meaning.strip()))
        else:
    ...
    for index, (pattern, meaning) in enumerate(entries):
        row = index // cols
        col = index % cols
        ...
        draw_text(draw, (box_x + 20, box_y + 14), pattern.upper(), "#4b5b73", 22, True)
        draw_text(draw, (box_x + 20, box_y + 48), meaning, "#111827", 29, False)
...
def main():
    ...
    draw_text(draw, (MARGIN, CARD_H - 70),
