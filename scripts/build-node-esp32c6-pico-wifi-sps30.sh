...
NODE_DIR="${PROJECT_ROOT}/nodes/esp32c6Pico/matter-wifi-sps30-node"
...
start_epoch="$(date +%s)"
echo ">>> Building ESP32-C6-Pico Matter Wi-Fi SPS30 firmware (timeout=${BUILD_TIMEOUT_SEC}s)..."
...
printf '>>> Firmware build ready in %02d:%02d:%02d\n' \
  ...
  "$(((elapsed_sec % 3600) / 60))" \
  "$((elapsed_sec % 60))"
echo ">>> App image: ${NODE_DIR}/build/esp32c6_pico_matter_wifi_sps30_node.bin"
