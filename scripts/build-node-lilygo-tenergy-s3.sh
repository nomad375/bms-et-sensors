...
NODE_DIR="${PROJECT_ROOT}/nodes/lilygoTEnergyS3/matter-wifi-node"
...
start_epoch="$(date +%s)"
echo ">>> Building LILYGO T-Energy-S3 Matter Wi-Fi firmware (timeout=${BUILD_TIMEOUT_SEC}s)..."
...
(
  ...
  idf_args=()
  if [[ "${BMS_RGB_LED_GPIO:-}" != "" ]]; then
    idf_args+=("-D" "BMS_RGB_LED_GPIO=${BMS_RGB_LED_GPIO}")
  fi
  timeout "${BUILD_TIMEOUT_SEC}" idf.py "${idf_args[@]}" build
)
status=$?
set -e
...
printf '>>> Firmware build ready in %02d:%02d:%02d\n' \
  ...
  "$(((elapsed_sec % 3600) / 60))" \
  "$((elapsed_sec % 60))"
echo ">>> App image: ${NODE_DIR}/build/lilygo_tenergy_s3_matter_wifi_node.bin"
echo ">>> Matter OTA image: ${NODE_DIR}/build/lilygo_tenergy_s3_matter_wifi_node-ota.bin"
